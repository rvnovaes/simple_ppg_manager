<!-- prompt.md — o prompt entregue ao Amp a cada iteração (`helton.sh` sem
     `--tool claude`). É o GÊMEO do CLAUDE.md ao lado: mesmo filtro de gates,
     mesma contabilidade, mesmas regras.

     NÃO substitua este arquivo pelo prompt.md do snarktank/ralph. O upstream não
     conhece human_gate nem blocked_by_gate — um loop rodando com aquele prompt
     executa as stories barradas, que é exatamente o que a esteira existe para
     impedir. Ele também manda carregar a skill `dev-browser`, que pode não
     existir no seu ambiente.

     Ao mudar um dos dois arquivos, mude o outro. -->
...
6. Run the quality checks the root rules file lists under **Comandos**
7. Update the nearby rules files (`CLAUDE.md`/`AGENTS.md`) if you discover reusable patterns (see below)
...
The repository root rules file (`CLAUDE.md` for Claude Code, `AGENTS.md` for Amp) is the single source of truth for branch naming, base branch,
...
## Update the rules files (CLAUDE.md / AGENTS.md)
